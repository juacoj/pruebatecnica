...
st.header("Clientes encontrados: ")
df['TIPO_CLIENTE'] = df['TIPO_CLIENTE'].replace({1: 'Regulado', 0: 'Libre'})
df = df.rename(columns={
    'CONCESIONARIA': 'Concesionaria',
    'TIPO_CLIENTE': 'Tipo Cliente',
    'CLIENTE_ID': 'Cliente ID',
    'NOMBRE O RAZÓN SOCIAL DEL CLIENTE': 'Nombre/Razón Social',
    'S/E PRIMARIA': 'S/E Primaria',
    'TIPO_EMPALME_ID': 'Tipo de Empalme',
    'POTENCIA_CONECTADA [kW]': 'Potencia Conectada (kW)',
    'CONSUMO PROMEDIO ÚLTIMOS 12 MESES [kWh]': 'Consumo Últimos 12 Meses (kWh)'
})
tabs = st.tabs(["Cliente 1", "Cliente 2", "Cliente 3", "Cliente 4", "Cliente 5"])
with tabs[0]:
    st.dataframe(df.iloc[6297], width= 900)
with tabs[1]:
    st.dataframe(df.iloc[928], width= 900)
with tabs[2]:
    st.dataframe(df.iloc[9025], width= 900)
with tabs[3]:
    st.dataframe(df.iloc[1682], width= 900)
with tabs[4]:
    st.dataframe(df.iloc[4076], width= 900)
st.caption("Cabe destacar que el 75% de los clientes regulados que corresponden a la concesionaria de la compañía eléctrica de Osorno no superan un consumo de 337 kWh en promedio de los últimos 12 meses")